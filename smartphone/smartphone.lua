...
local function drawiPhone()
    ...
    ui.childWindow('OnTopImages', vec2(app.images.phoneAtlasSize.x / 3, app.images.phoneAtlasSize.y), false, WINDOWFLAGS, function()
        ui.drawImage(app.images.phoneAtlasPath, vec2(0, movement.smooth), vec2(app.images.phoneAtlasSize.x / 3, app.images.phoneAtlasSize.y + movement.smooth), rgb.colors.white, vec2(1 / 3, 0), vec2(2 / 3, 1))
        if not settings.darkMode then
            ui.drawImage(app.images.phoneAtlasPath, vec2(scale(2), movement.smooth), vec2(app.images.phoneAtlasSize.x / 3, app.images.phoneAtlasSize.y + movement.smooth), rgb.colors.white, vec2(2 / 3, 0), vec2(3 / 3, 1))
        end
    end)
end
...
local function drawHeader()
    ...
    ui.popDWriteFont()
    ui.drawImage(app.images.phoneAtlasPath, vec2(0, movement.smooth), vec2(app.images.phoneAtlasSize.x / 3, app.images.phoneAtlasSize.y + movement.smooth), rgb.colors.white, vec2(0 / 3, 0), vec2(1 / 3, 1))

    local avatarSize = vec2(36, 36):scale(app.scale)
    local avatarPosition = vec2(129, 47):scale(app.scale) + vec2(0, movement.smooth)
    if player.isOnline and player.serverCommunity == 'srp' then
        ui.drawImageRounded(app.images.communities.srpLogo, avatarPosition, avatarPosition + avatarSize, scale(20))
    end
end
...
function script.windowMain(dt)
    ...
    ui.childWindow('Phone', vec2(app.images.phoneAtlasSize.x / 3, app.images.phoneAtlasSize.y), false, WINDOWFLAGS, function()
        drawDisplay()
